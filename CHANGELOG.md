...
- Company Sources (`/company-sources`, Collect → Manage sources): one table for companies and vertical channels. `source_registry.kind` is `company`, `wechat`, `community`, or `other`. YAML seeds companies once; user-added rows persist. Auto Collect still runs only enabled + this-run company adapters.
