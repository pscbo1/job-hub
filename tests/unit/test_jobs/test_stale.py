"""14d stale-applied smart filter."""
...
from job_sentinel.jobs.membership import is_stale_applied
...
def test_stale_applied_skips_next_step_task_and_follow_up() -> None:
    old = _old()
    ...
    assert is_stale_applied(app, with_next) is False
    with_task = Job(
        source="x",
        source_job_id="2",
    ...
    assert is_stale_applied(app, with_task) is False
    future = datetime.now(tz=UTC) + timedelta(days=2)
    with_follow = Job(source="x", source_job_id="3", follow_up_at=future, last_activity_at=old)
    assert is_stale_applied(app, with_follow) is False
