  ...
  useEffect(() => {
    const load = async () => {
      const results = await Promise.all([
        fetch(`${API_BASE}/api/jobs?limit=1`).then((r) => (r.ok ? r.json() : null)).catch(() => null),
        fetch(`${API_BASE}/api/applications?limit=1`).then((r) => (r.ok ? r.json() : null)).catch(() => null),
        fetch(`${API_BASE}/api/tasks?limit=1`).then((r) => (r.ok ? r.json() : null)).catch(() => null),
      ]);
      const jobs = Array.isArray(results[0]) ? results[0] : results[0]?.items ?? [];
    ...
    <main className="mx-auto max-w-5xl px-5 py-12">
      <header className="mb-8">
        ...
        <p className="mt-1 text-xs text-muted">
          {hasWorkspace
            ? "These are suggestions based on your workspace. They do not change your records automatically."
            : "These are suggestions for getting started. They do not change your records automatically."}
        </p>
      </header>
