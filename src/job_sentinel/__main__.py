...
def archive(
    ...
    """Close idle Job Pool rows (cron-friendly). No-op unless enabled, unless --force.

    Settings: GET/PUT /api/archive-settings (enabled default off, idle_days=14).
    """
    from job_sentinel.config.settings import get_settings
    from job_sentinel.db.repository import JobRepository
    from job_sentinel.jobs.archive import load_archive_settings, run_auto_archive

    settings = get_settings()
    repo = JobRepository(settings.db_path)
    try:
        cfg = load_archive_settings(repo)
        result = run_auto_archive(repo, settings=cfg, dry_run=dry_run, force=force)
    finally:
        repo.close()
    if result.disabled:
        console.print(
    ...
    console.print(
        ...
        f"closed={result.archived} skipped={result.skipped}"
    )
